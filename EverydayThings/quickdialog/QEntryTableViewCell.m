    ...
    if (CGRectEqualToRect(CGRectZero, _entryElement.parentSection.entryPosition)) {
        for (QElement *el in _entryElement.parentSection.elements){
            if ([el isKindOfClass:[QEntryElement class]]){
                ...
                NSAttributedString *attributedText = [[NSAttributedString alloc] initWithString:((QEntryElement *)el).title
                                                                                     attributes:@{ NSFontAttributeName: [self.textLabel.font fontWithSize:fontSize]} ];
