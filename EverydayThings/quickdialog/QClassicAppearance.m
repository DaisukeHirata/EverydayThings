...
- (UIView *)buildFooterForSection:(QSection *)section andTableView:(QuickDialogTableView *)tableView andIndex:(NSInteger)index {
    ...
    if (self.sectionFooterFont!=nil && tableView.style == UITableViewStyleGrouped){
        CGRect textRect = [section.footer boundingRectWithSize:CGSizeMake(tableView.bounds.size.width-margin-margin, 1000000)
                                             options:NSStringDrawingUsesLineFragmentOrigin
                                          attributes:@{NSFontAttributeName:self.sectionFooterFont}
                                             context:nil];
...
- (CGFloat)heightForHeaderInSection:(QSection *)section andTableView:(QuickDialogTableView *)tableView andIndex:(NSInteger)index {
    ...
    if (!tableView.root.grouped)  {
        ...
        CGRect textRect = [section.title boundingRectWithSize:CGSizeMake(tableView.bounds.size.width-margin-margin, 1000000)
                                             options:NSStringDrawingUsesLineFragmentOrigin
                                          attributes:@{NSFontAttributeName:self.sectionTitleFont}
                                             context:nil];
    ...
    if (section.title != nil) {
        ...
        CGSize expectedLabelSize = [section.title boundingRectWithSize:maximumLabelSize
                                                               options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin
                                                            attributes:@{ NSFontAttributeName :
                                                                              appearance==nil? [UIFont systemFontOfSize:[UIFont labelFontSize]] : appearance.sectionTitleFont }
                                                               context:nil].size;
...
- (CGFloat)heightForFooterInSection:(QSection *)section andTableView:(QuickDialogTableView *)tableView andIndex:(NSInteger)index {
    ...
    CGRect textRect = [section.footer boundingRectWithSize:CGSizeMake(tableView.bounds.size.width-margin-margin, 1000000)
                                         options:NSStringDrawingUsesLineFragmentOrigin
                                      attributes:@{NSFontAttributeName:appearance.sectionFooterFont}
                                         context:nil];
